...
--Cau 1: Xem nội dung của tất cả các bảng dữ liệu bằng lệnh SELECT
--
select *
from chuongtrinh;
--
...
--Cau 2: Tìm thông tin về các học viên nam

select *
from hocvien
where gioitinh like '1';

--Cau 3: Tìm thông tin về các học viên có địa chỉ ở Cần Thơ

select *
from hocvien
where diachi like '%Cần Thơ%';

--Cau 4: Tìm thông tin về các lớp học của ‘khoá 1’

select l.makh,malop,maloai,tenlop,siso
from lop l, khoahoc kh
where kh.makh=l.makh
and l.makh='K001';

--Cau 5: Tìm mã và họ tên học viên có học ‘khoá 1’

select hv.mahv, hv.tenhv
from lop l, khoahoc kh, hocvien hv, diem d
where kh.makh=l.makh
and hv.mahv=d.mahv
and d.malop=l.malop
and l.makh='K001';

--Cau 6: Tìm họ tên các học viên có bao gồm chữ ‘Đỗ’

select *
from hocvien
where tenhv like '%Đỗ%';

--Cau 7: Tìm thông tin các học viên sinh năm 2000 ?

select *
from hocvien
where extract(year from ngaysinh) ='2000';

--Cau 8: Tìm thông tin của các học viên sinh tháng 12 năm 2001 ?
...
--Cau 9: Tìm thông tin các học viên sinh từ năm 1998 đến 2000

select *
from hocvien
where extract(year from ngaysinh) between 1998 and 2000;

--Cau 10: Tìm thông tin các phiếu thu được thực hiện từ ngày 5 đến ngày 10 tháng 6 năm 2021?

select *
from phieuthu
where extract(day from ngaylapphieu) between 5 and 10
and extract(month from ngaylapphieu)='6'
and extract(year from ngaylapphieu)='2021';

--Cau 11: In danh sách các học viên lớp ‘Lớp 1’ Tiếng anh căn bản

select hv.*
from hocvien hv, lop l, loailop ll, diem d
where hv.mahv=d.mahv
and l.maloai=ll.maloai
and d.malop=l.malop
and l.tenlop='Lớp 1'
and ll.tenloai='Tiếng Anh căn bản';

--Cau 12: In danh sách các lớp thuộc chương trình ‘Tiếng anh tổng quát’

select l.*
from lop l, loailop ll, chuongtrinh ct
where l.maloai = ll.maloai
and ll.mact=ct.mact
and ct.tenct='Tiếng Anh Tổng Quát';

--Cau 13: Liệt kê thông tin tất cả các phiếu thu của ‘lớp 1’ Tiếng anh A1?

select pt.*
from phieuthu pt, lop l, loailop ll
where pt.malop=l.malop
and l.maloai=ll.maloai
and l.tenlop='Lớp 1'
and ll.tenloai='Tiếng Anh A1';

--Cau 14: Tìm họ tên học viên, tên môn và điểm thi các môn của các học viên học ‘khoá 1’

select hv.tenhv, mh.tenmh, d.diem
from hocvien hv, monhoc mh, diem d, khoahoc kh, lop l
where hv.mahv=d.mahv
and hv.mahv=d.mahv
and l.makh=kh.makh
and d.malop=l.malop
...
--Cau 15: Có tất cả bao nhiêu học viên?

select count(mahv) as "So hoc vien"
from hocvien;

--Cau 16: ‘Lớp 1’ Tiếng anh căn bản có bao nhiêu học viên?

select count(hv.mahv) as "So hoc vien"
from lop l, loailop ll, hocvien hv, diem d
where l.maloai=ll.maloai
and hv.mahv=d.mahv
and d.malop=l.malop
and l.tenlop='Lớp 1'
...
--Cau 17: Tính tổng số tiền đã thu được của ‘lớp 2’ Tiếng anh căn bản

select sum(pt.thanhtien) as "Tong so tien"
from lop l, loailop ll, phieuthu pt
where l.maloai=ll.maloai
and l.malop=pt.malop
and l.tenlop='Lớp 2'
and ll.tenloai='Tiếng Anh căn bản';

--Cau 18: Tính tổng số tiền đã thu được của ‘khoá 1’ ?

select sum(pt.thanhtien) as "Tong so tien"
from phieuthu pt, khoahoc kh, lop l
where pt.malop=l.malop
and l.makh=kh.makh
and kh.tenkh='Khóa 1';

--Cau 19: Tính điểm trung bình của học viên 'Đỗ Gia Bảo', sinh ngày 02/12/2001 học ‘lớp 1’ Tiếng anh căn bản?

select avg(d.diem) as "Diem trung binh"
from hocvien hv, lop l, loailop ll, diem d
where hv.mahv=d.mahv
and l.malop=d.malop
and l.maloai=ll.maloai
and hv.tenhv='Đỗ Gia Bảo'
and l.tenlop='Lớp 1'
and ll.tenloai='Tiếng Anh căn bản'
and extract(day from hv.ngaysinh)='02'
and extract(month from hv.ngaysinh)='12'
and extract(year from hv.ngaysinh)='2001';

--Cau 20: Tìm điểm lớn nhất?

select max(diem) as "Diem lon nhat"
from diem;