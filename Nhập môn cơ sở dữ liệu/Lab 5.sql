...
--Cau 1: Vẽ sơ đồ thông thương của các quan hệ
select * from khuvuc;
select * from caidat;
select * from loai;
select * from may;
...
--Cau 2: Loại của máy 'p8'
select l.tenloai
from may m join loai l on m.idloai=l.idloai
where idmay='p8';

--Cau 3: Tên của các phần mềm 'UNIX'
select tenpm
from phanmem
where idloai='UNIX';

--Cau 4: Tên phòng, địa chỉ IP phòng, mã phòng của các máy loại 'UNIX' hoặc 'PCWS'
select DISTINCT p.tenphong,p.mp,p.ip
from phong p join may m on p.ip=m.ip
where m.idloai='UNIX'
or m.idloai='PCWS';

--Cau 5: Tên phòng, địa chỉ IP phòng, mã phòng của các máy loại 'UNIX' hoặc 'PCWS' ở khu vực '130.120.80', sắp xếp kết quả tăng dần theo mã phòng
select MP, tenphong,IP
from phong
where ip='130.120.80'
and mp in (select DISTINCT p.mp
            from phong p join may m on p.ip=m.ip
            where m.idloai='UNIX'
            or m.idloai='PCWS');

--Cau 6: Số các phần mềm được cài đặt trên máy 'p6'
select m.idmay,count(*) as "So cac phan mem"
from may m join phanmem pm on m.idloai=pm.idloai
where m.idmay='p6'
group by m.idmay;

--Cau 7: Số các máy đã cài phần mềm 'log1'
select count(m.idmay) as "So may"
from may m join phanmem pm on m.idloai=pm.idloai
where pm.idpm='log1';

--Cau 8: Tên và địa chỉ IP (ví dụ: 130.120.80.1) đầy đủ của các máy loại 'TX'
select ip || '.' || ad as "enough ip"
from may
where idloai='TX';

--Cau 9: Tính số phần mềm đã cài đặt trên mỗi máy
select idMay,count(ngaycai)
from Caidat
group by idMay;

--Cau 10: Tính số máy mỗi phòng
select tenphong, somay
from phong;

--Cau 11: Tính số cài lần cài đặt của mỗi phần mềm trên các máy khác nhau
select idMay,count(id)
from Caidat
group by idMay;

--Cau 12: Giá trung bình của các phần mềm UNIX
select idloai, avg(gia) as "Gia trung binh"
from phanmem
where idloai='UNIX'
group by idloai;

--Cau 13: Ngày mua phần mềm gần nhất
select max(ngaymua)
from phanmem;

--Cau 14: Số máy có ít nhất 2 phần mềm
select count(*) as "So may"
from (select idmay
    from caidat
    group by idmay
    having count(idpm)>=2);
...
select m.ip || '.' || m.ad as "IP day du"
from phanmem pm, caidat cd, may m
where pm.idpm=cd.idpm
and m.idmay=cd.idmay
and pm.idpm='log6'
and cd.ngaycai is not NULL;
...
where mp in(
    select m.mp
    from caidat cd,may m,phanmem pm
    where cd.idmay=m.idmay
    and cd.idpm=pm.idpm
    and pm.tenpm='Oracle 6');

--Cau 22: Tên phần mềm được mua gần nhất
select tenpm
from phanmem
where ngaymua=(
    select max(ngaymua)
...
--Cau 23: Tên của phần mềm PCNT có giá lớn hơn bất kỳ giá của một phần mềm UNIX nào
select tenpm
from phanmem
where idloai='PCNT'
and gia > any(
...
--Cau 24: Tên của phần mềm UNIX có giá lớn hơn tất cả các giá của các phần mềm PCNT
select tenpm
from phanmem
where idloai='UNIX'
and gia > all(
...
--Cau 25: Tên của máy có ít nhất một phần mềm chung với máy 'p6'
select tenmay
from may
where idmay in(
    select DISTINCT idmay
    from caidat
    where idpm in (select idpm from caidat where idmay = 'p6')
    and idmay != 'p6');

--Cau 26: Tên của các máy có cùng phần mềm như máy 'p6' (có thể nhiều phần mềm hơn máy 'p6')
select tenmay
from may
where idmay in(
    select cd.idmay
    from caidat cd join (select idpm from caidat where idmay='p6') c2 on cd.idpm=c2.idpm
    where cd.idmay != 'p6'
    group by cd.idmay
    having count(*) = (select count(idpm) from caidat where idmay='p6'));
    
--Cau 27: Tên của các máy có chính xác các phần mềm như máy 'p2'
select tenmay
from may
where idmay in (
    select idmay
    ...
    where idmay in (
        select cd.idmay
        from caidat cd join (select idpm from caidat where idmay='p2') c2 on cd.idpm=c2.idpm
        where cd.idmay != 'p2'
        group by cd.idmay
        having count(*) = (select count(idpm) from caidat where idmay='p2'))
    group by idmay
